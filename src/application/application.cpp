...
void Application::update_style() {
  ...
  float view_distance = length(camera.position() - camera.view_point());
...
void Application::update_gl_camera() {
  ...
  const Vector3D& c = to_Vector3D(camera.position());
  const Vector3D& r = to_Vector3D(camera.view_point());
  const Vector3D& u = to_Vector3D(camera.up_dir());
...
void Application::load(SceneInfo* sceneInfo) {
  ...
  if (!bbox.empty()) {
    ...
    double3 target = bbox.centroid();
    canonical_view_distance = length(bbox.hi - bbox.lo) / 2 * 1.5;
    ...
    canonicalCamera.place(to_Vector3D(target),
                          acos(c_dir.y),
    ...
    camera.place(to_Vector3D(target),
                acos(c_dir.y),
                atan2(c_dir.x, c_dir.z),
                view_distance,
                min_view_distance,
