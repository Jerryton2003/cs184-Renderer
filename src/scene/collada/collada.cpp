    ...
    if (tech_CGL) {
      ...
      while (e_bsdf) {
        ...
        if (type == "emission") {
          ...
          BSDF* bsdf = new EmissionBSDF(to_double3(radiance));
          material.bsdf = bsdf;
        } else if (type == "mirror") {
          ...
          BSDF* bsdf = new MirrorBSDF(to_double3(reflectance));
          material.bsdf = bsdf;
          ...
          BSDF* bsdf = new MicrofacetBSDF(to_double3(eta), to_double3(k), alpha);
          material.bsdf = bsdf;
          ...
          BSDF* bsdf = new RefractionBSDF(to_double3(transmittance), roughness, ior);
          material.bsdf = bsdf;
        } else if (type == "glass") {
          XMLElement *e_transmittance  = get_element(e_bsdf, "transmittance");
          XMLElement *e_reflectance  = get_element(e_bsdf, "reflectance");
          ...
          float roughness = atof(e_roughness->GetText());
          float ior = atof(e_ior->GetText());
          BSDF* bsdf = new GlassBSDF(to_double3(transmittance), to_double3(reflectance), roughness, ior);
          material.bsdf = bsdf;
        }
      ...
      if (e_diffuse) {
        ...
        material.bsdf = new DiffuseBSDF(to_double3(reflectance));
      } else {
        material.bsdf = new DiffuseBSDF(make_double3(.5f,.5f,.5f));
      }
    } else {
      BSDF* bsdf = new DiffuseBSDF(make_double3(.5f,.5f,.5f));
      material.bsdf = bsdf;
    }
  } else {
