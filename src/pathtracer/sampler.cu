#include "sampler.h"
#include "../util/gpu-arrays.h"
namespace CGL {

/**
 * A Sampler2D implementation with uniform distribution on unit square
 */
double2 UniformGridSampler2D::get_sample() const {

  return double2(random_uniform(), random_uniform());

}


// Uniform Sphere Sampler3D Implementation //

double3 UniformSphereSampler3D::get_sample() const {
  double z = random_uniform() * 2 - 1;
  double sinTheta = sqrt(std::max(0.0, 1.0f - z * z));

  double phi = 2.0f * PI * random_uniform();

  return double3(cos(phi) * sinTheta, sin(phi) * sinTheta, z);
}


/**
 * A Sampler3D implementation with uniform distribution on unit hemisphere
 */
double3 UniformHemisphereSampler3D::get_sample() const {

  double Xi1 = random_uniform();
  double Xi2 = random_uniform();

  double theta = acos(Xi1);
  double phi = 2.0 * PI * Xi2;

  double xs = sinf(theta) * cosf(phi);
  double ys = sinf(theta) * sinf(phi);
  double zs = cosf(theta);

  return double3(xs, ys, zs);

}

/**
 * A Sampler3D implementation with cosine-weighted distribution on unit
 * hemisphere. This function does not return the pdf.
 */
double3 CosineWeightedHemisphereSampler3D::get_sample() const {
  double f;
  return get_sample(&f);
}

/**
 * A Sampler3D implementation with cosine-weighted distribution on unit
 * hemisphere. This functions also sets the pdf to the proper probability
 */
double3 CosineWeightedHemisphereSampler3D::get_sample(double *pdf) const {

  double Xi1 = random_uniform();
  double Xi2 = random_uniform();

  double r = sqrt(Xi1);
  double theta = 2. * PI * Xi2;
  *pdf = sqrt(1-Xi1) / PI;
  return double3(r*cos(theta), r*sin(theta), sqrt(1-Xi1));
}


} // namespace CGL
